            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-700">
              <table className="min-w-full divide-y divide-slate-100 text-sm dark:divide-slate-800">
                <thead className="bg-slate-50 dark:bg-slate-950/40">
                  <tr>
                    <th className="px-4 py-2.5 text-left text-xs font-medium text-slate-500">Désignation</th>
...
export default function CommercialDeliveriesPage() {
  ...
  const loadPending = async () => {
    try {
      ...
      setError("");
      const [ords, deps] = await Promise.all([
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-violet-50 dark:bg-violet-900/30">
            <Truck size={18} className="text-violet-600 dark:text-violet-400" />
          </div>
          <div>
        ...
        <div className="flex gap-1 rounded-2xl border border-slate-200 bg-slate-50 p-1 dark:border-slate-800 dark:bg-slate-900/60">
          ...
          >
            ...
            En attente
            {orders.length > 0 && (
              <span className="rounded-full bg-violet-100 px-1.5 py-0.5 text-[10px] font-semibold text-violet-700 dark:bg-violet-900/40 dark:text-violet-400">
                {orders.length}
              </span>
            )}
          </button>
                  ...
                  <table className="min-w-full divide-y divide-slate-100 text-sm dark:divide-slate-800">
                    <thead className="bg-slate-50 dark:bg-slate-950/40">
                      <tr>
                        ...
                        <th className="px-5 py-3 text-left font-medium text-slate-500">Désignations</th>
                        ...
                        <tr key={order._id} className="transition hover:bg-slate-50 dark:hover:bg-slate-800/40">
                          ...
                          <td className="px-5 py-3.5 text-right">
                            <button
                              ...
                              className="flex items-center gap-1.5 rounded-xl bg-violet-600 px-3 py-1.5 text-xs font-medium text-white transition hover:bg-violet-700"
                            >
                              <Truck size={12} />
                              Réceptionner
                            </button>
