...
import { purchaseDocumentService, type PurchaseDocument, type DocumentStats } from "@/services/purchase/purchaseDocumentService";
...
export default function PurchaseDocumentsPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [d, s] = await Promise.all([purchaseDocumentService.getAll(), purchaseDocumentService.getStats()]);
      setDocs(d);
      setStats(s);
    } catch (e: unknown) {
  ...
  const handleUpload = async (files: FileList | null) => {
    ...
    setUploading(true);
    try {
      await purchaseDocumentService.upload(files[0]);
      await load();
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message;
  ...
  const confirmDelete = async () => {
    ...
    try {
      await purchaseDocumentService.delete(otpTarget, otpInput);
      setDocs((prev) => prev.filter((d) => d._id !== otpTarget));
      void purchaseDocumentService.getStats().then(setStats).catch(() => {});
      setOtpTarget(null);
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { message?: string } } })?.response?.data?.message;
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white">
              Purchase <span className="text-teal-500">Documents</span>
            </h1>
                ...
                {docs.length === 0 ? (
                  ...
                  <div key={doc._id} className="flex items-center gap-3 px-5 py-3.5 transition hover:bg-slate-50 dark:hover:bg-slate-800/20">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                      <button
                        onClick={() => purchaseDocumentService.download(doc._id, doc.originalName)}
                        title="Télécharger"
