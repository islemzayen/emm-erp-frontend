...
import {
  ...
  Filter, ChevronDown, Loader2, Clock, ShieldAlert, FileSignature,
} from "lucide-react";
import { documentService } from "@/services/documentService";
import { generateAttestation } from "@/lib/attestationDocument";
import api from "@/services/api";
import { hrService } from "@/services/hrservice";
import deleteRequestService from "@/services/deleteRequestService";

const DOC_TYPES = ["Contract", "Absence Reason", "Resignation", "Warning", "Medical Certificate", "ID Copy", "Attestation de travail", "Other"];

const TYPE_COLORS: Record<string, string> = {
  "Contract":             "bg-blue-500/10 text-blue-400 border-blue-500/20",
  "Absence Reason":       "bg-amber-500/10 text-amber-400 border-amber-500/20",
  "Resignation":          "bg-red-500/10 text-red-400 border-red-500/20",
  "Warning":              "bg-orange-500/10 text-orange-400 border-orange-500/20",
  "Medical Certificate":  "bg-purple-500/10 text-purple-400 border-purple-500/20",
  "ID Copy":              "bg-cyan-500/10 text-cyan-400 border-cyan-500/20",
  "Attestation de travail": "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
  "Other":                "bg-gray-500/10 text-gray-400 border-gray-500/20",
};

interface Doc {
  _id: string; employeeId: string; employeeName: string;
  department: string; type: string; fileName: string;
  fileSize: number; note: string; uploadedBy: string; createdAt: string;
}
interface Employee { _id: string; name: string; department: string; role: string; }
...
export default function HRDocumentsPage() {
  ...
  const [form, setForm]               = useState({ employeeId: "", type: DOC_TYPES[0], note: "" });
  ...
  const [genForm, setGenForm]     = useState({ personId: "" });
  ...
  const inputClass = "w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder:text-gray-500 focus:outline-none focus:border-[#c8202f]/60 transition";
  ...
  async function fetchAll() {
    ...
    try {
      ...
      setEmployees(unwrap(empsRes).filter((e: any) => ["EMPLOYEE", ""].includes((e.role || "").toUpperCase())));
      setPeople(unwrap(empsRes));
    } catch {}
    try { setDocs(unwrap(await documentService.list())); } catch {}
    try { const r: any = await api.get("/company-config"); setCompany(r?.data?.data ?? r?.data ?? null); } catch {}
    setLoading(false);
  }
  ...
  async function handleUpload() {
    if (!form.employeeId) { setUploadError("Please select an employee."); return; }
    if (!file)            { setUploadError("Please select a PDF file."); return; }
    setUploading(true); setUploadError("");
    try {
      const emp = employees.find(e => e._id === form.employeeId);
      const result = await documentService.upload({
        employeeId: form.employeeId, employeeName: emp?.name ?? "",
        department: emp?.department ?? "HR", type: form.type, note: form.note, file,
      });
      if (result && !result.message?.toLowerCase().includes("error") && !result.statusCode) {
        setShowModal(false);
        setForm({ employeeId: "", type: DOC_TYPES[0], note: "" });
        setFile(null); setEmpSearch(""); setShowEmpList(false);
        fetchAll();
  ...
  async function handleGenerate() {
    const person: any = people.find(p => p._id === genForm.personId);
    if (!person) { setGenError("Please select an employee or manager."); return; }
    setGenLoading(true); setGenError("");
    try {
      const { blob, fileName } = await generateAttestation({
        employee: {
          name: person.name, position: person.position,
          cin: person.cin, joinedDate: person.joinedDate,
        },
        company: company ? { representative: company.representative, place: company.place || company.city } : undefined,
      });
      const pdf = new File([blob], fileName, { type: "application/pdf" });
      const result = await documentService.upload({
        employeeId: person._id, employeeName: person.name,
        department: person.department || "HR", type: "Attestation de travail",
        note: "Attestation de travail",
        file: pdf,
      });
      if (result && !result.statusCode && !result.message?.toLowerCase?.().includes("error")) {
        setShowGen(false);
        setGenForm({ personId: "" });
        fetchAll();
      } else { setGenError(result?.message || "Generation failed."); }
    } catch (err: any) {
  ...
  // ── Filters ──────────────────────────────────────────────────────────────
  const departments = ["All", ...Array.from(new Set(docs.map(d => d.department).filter(Boolean)))];
  const types       = ["All", ...DOC_TYPES];
  const filtered    = docs.filter(d => {
    const matchSearch = d.employeeName.toLowerCase().includes(search.toLowerCase()) || d.fileName.toLowerCase().includes(search.toLowerCase());
    return matchSearch && (filterType === "All" || d.type === filterType) && (filterDept === "All" || d.department === filterDept);
  });
  ...
  return (
    <ProtectedRoute allowedRoles={["HR_MANAGER"]}>
                ...
                {deleteStep === "awaiting" && (
                  <div className="space-y-4">
                    ...
                    <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl px-4 py-3">
                      ...
                      <p className="text-[10px] text-gray-500 mt-1">{deleteTarget.employeeName} · {deleteTarget.department}</p>
                    </div>
                    ...
                    <div className="flex gap-2 justify-center">
                      {Array.from({ length: 6 }).map((_, i) => (
                        <input key={i} id={`code-${i}`} type="text" inputMode="numeric" maxLength={1}
                          ...
                          onKeyDown={e => { if (e.key === "Backspace" && !deleteCode[i] && i > 0) (document.getElementById(`code-${i - 1}`) as HTMLInputElement)?.focus(); }}
      ...
      <div className="min-h-screen bg-[#f0f4ff] dark:bg-[#060d1f] text-gray-900 dark:text-white font-mono p-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">
              Employee <span className="text-[#c8202f]">Documents</span>
            </h1>
            <p className="text-xs text-gray-500 mt-1.5 uppercase tracking-widest">EMM ERP · HR</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => { setShowGen(true); setGenError(""); setGenForm({ personId: "" }); }}
              className="flex items-center gap-2 border border-[#c8202f] text-[#c8202f] hover:bg-[#c8202f] hover:text-white px-4 py-2 rounded-xl text-xs uppercase tracking-wide transition font-bold">
              <FileSignature size={13} /> Generate Attestation
            </button>
            <button
              onClick={() => { setShowModal(true); setUploadError(""); setFile(null); setEmpSearch(""); setShowEmpList(false); setForm({ employeeId: "", type: DOC_TYPES[0], note: "" }); }}
        ...
        {/* ── Generate Attestation Modal ── */}
        <AnimatePresence>
          {showGen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
              <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
                className="bg-white dark:bg-[#111c35] border border-[#1b2a6b]/15 dark:border-[#1b2a6b]/20 rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-white/[0.08]">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-[#c8202f]/10 flex items-center justify-center"><FileSignature size={15} className="text-[#c8202f]" /></div>
                    <h2 className="text-sm font-bold text-gray-900 dark:text-white">Generate Attestation de travail</h2>
                  </div>
                  <button onClick={() => setShowGen(false)} className="text-gray-500 hover:text-white transition"><X size={18} /></button>
                </div>
                <div className="p-6 space-y-4">
                  <div>
                    <label className={labelClass}>Employee / Manager</label>
                    <select className="w-full rounded-xl px-3 py-2 text-sm bg-gray-100 dark:bg-white/5 border border-gray-300 dark:border-white/10 text-gray-900 dark:text-white focus:outline-none focus:border-[#c8202f]/60 transition" value={genForm.personId}
                      onChange={e => setGenForm(f => ({ ...f, personId: e.target.value }))}>
                      <option value="">— Select a person —</option>
                      {people.map(pp => {
                        const x = pp as any;
                        return (
                          <option key={pp._id} value={pp._id}>
                            {x.name}{x.position ? ` — ${x.position}` : ""}{x.role && x.role.toUpperCase() !== "EMPLOYEE" ? ` (${x.role})` : ""}
                          </option>
                        );
                      })}
                    </select>
                  ...
                  <div className="flex gap-2 pt-2">
                    <button onClick={handleGenerate} disabled={genLoading || !genForm.personId}
                      className="flex-1 flex items-center justify-center gap-2 bg-[#c8202f] hover:bg-[#e02d3c] disabled:opacity-50 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wide text-white transition">
                      {genLoading ? <><Loader2 size={14} className="animate-spin" /> Generating…</> : <>Generate &amp; Save</>}
                    </button>
                    <button onClick={() => setShowGen(false)} className="px-4 py-2.5 rounded-xl text-xs text-gray-500 hover:text-white border border-white/10 transition">Cancel</button>
                  </div>
                  <p className="text-[10px] text-gray-500">Saved automatically to this Documents list as an “Attestation de travail”.</p>
                </div>
              </motion.div>
            </motion.div>
          )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Employees with Docs", value: new Set(docs.map(d => d.employeeId)).size },
        ...
        <div className={`${card} p-4 flex flex-wrap gap-3 items-center`}>
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input className="w-full pl-8 pr-4 py-2 bg-white dark:bg-[#111c35] border border-[#1b2a6b]/15 dark:border-[#1b2a6b]/20 rounded-xl text-xs text-gray-900 dark:text-white focus:outline-none focus:border-[#c8202f]/60 transition placeholder:text-gray-400"
              placeholder="Search employee or file name…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          ...
          <div className="relative">
            <select value={filterDept} onChange={e => setFilterDept(e.target.value)}
              className="pl-4 pr-8 py-2 bg-white dark:bg-[#111c35] border border-[#1b2a6b]/15 dark:border-[#1b2a6b]/20 rounded-xl text-xs text-gray-900 dark:text-white focus:outline-none appearance-none transition">
            ...
            {filtered.map(doc => (
              <motion.div key={doc._id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <p className="text-xs text-gray-500 truncate">
                  {doc.employeeName} <span className="text-gray-300 dark:text-gray-600">·</span> {doc.department}
                </p>
      ...
      <AnimatePresence>
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              className="bg-white dark:bg-[#111c35] border border-[#1b2a6b]/15 dark:border-[#1b2a6b]/20 rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between">
                <h2 className="text-base font-bold text-gray-900 dark:text-white">Import Document</h2>
              ...
              <div className="relative">
                ...
                <div className="relative">
                  ...
                  <input className="w-full pl-8 pr-4 py-2 bg-white/5 border border-white/10 rounded-xl text-sm text-white focus:outline-none focus:border-[#c8202f]/60 transition placeholder:text-gray-500"
                    placeholder="Search employee..."
                    value={form.employeeId ? (employees.find(e => e._id === form.employeeId)?.name ?? empSearch) : empSearch}
                    onChange={e => { setEmpSearch(e.target.value); setForm(f => ({ ...f, employeeId: "" })); setShowEmpList(true); }}
                    onFocus={() => { if (!form.employeeId) setShowEmpList(true); }}
                    readOnly={!!form.employeeId}
                    onClick={() => { if (form.employeeId) { setForm(f => ({ ...f, employeeId: "" })); setEmpSearch(""); setShowEmpList(true); } }}
                  />
                  {form.employeeId && (
                    <button type="button" onClick={() => { setForm(f => ({ ...f, employeeId: "" })); setEmpSearch(""); setShowEmpList(false); }}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white transition"><X size={13} /></button>
                  )}
                </div>
                {showEmpList && !form.employeeId && (
                  <div className="absolute z-10 mt-1 w-full bg-white dark:bg-[#0a1020] border border-[#1b2a6b]/15 dark:border-[#1b2a6b]/20 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                    {employees.filter(e => e.name.toLowerCase().includes(empSearch.toLowerCase())).length === 0
                      ...
                      : employees.filter(e => e.name.toLowerCase().includes(empSearch.toLowerCase())).map(e => (
                        <button key={e._id} type="button"
                          onClick={() => { setForm(f => ({ ...f, employeeId: e._id })); setEmpSearch(""); setShowEmpList(false); }}
                          className="w-full text-left px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-white/5 transition flex items-center justify-between gap-2 group">
                          <span className="text-sm text-gray-900 dark:text-white font-medium group-hover:text-[#c8202f] transition">{e.name}</span>
                          <span className="text-[10px] text-gray-400 bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded-full">{e.department}</span>
                        </button>
                      ))
                    }
                  </div>
              ...
              <div>
                ...
                <input className={inputClass} placeholder="e.g. Sick leave - 3 days"
              ...
              <div>
                ...
                <div onClick={() => fileInputRef.current?.click()}
                  ...
                  {file ? (
                    ...
                    <div><Upload size={24} className="mx-auto text-gray-600 mb-2" /><p className="text-xs text-gray-400">Click to select a PDF</p><p className="text-[10px] text-gray-400 mt-0.5">Max 10MB</p></div>
                  )}
