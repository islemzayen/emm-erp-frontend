...
import {
  ...
  ShieldCheck, Clock, DollarSign, Save,
  FileText,
} from "lucide-react";
import { useState, useEffect, useRef } from "react";
import { adminService } from "@/services/adminService";
import { marketingService } from "@/services/marketingService";
import { budgetService } from "@/services/calendarService";
...
export default function AdminMarketingPage() {
  ...
  const [budgetCurrent, setBudgetCurrent] = useState<number | null>(null);
  ...
  const buildPayrollPayload = () => ({
    ...
    hourlyRate: Number(form.salary) ? Number((Number(form.salary) / 208).toFixed(3)) : 0,
    cin: form.cin,
  });
  ...
  useEffect(() => { fetchAll(); fetchBudget(budgetYear); }, []);

  const fetchBudget = async (year: number) => {
    try {
      const b = await budgetService.get(year);
      setBudgetCurrent(b.annualBudget);
      setAnnualBudget(b.annualBudget > 0 ? String(b.annualBudget) : "");
    } catch {}
  };

  const handleSaveBudget = async () => {
    const amount = Number(annualBudget);
    if (!amount || amount <= 0) return;
    setBudgetSaving(true);
    try {
      await budgetService.setAnnual(budgetYear, amount);
      setBudgetCurrent(amount);
      setBudgetSaved(true);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (!form.cin || form.cin.length !== 8) { setFormError("A valid 8-digit CIN number is required"); setSubmitting(false); return; }
      if (!cinFile) { setFormError("Please attach a copy of the CIN card (PDF or image)"); setSubmitting(false); return; }
      const created = await marketingService.createEmployee({
      ...
      if (newEmp?._id) {
        try {
          await documentService.upload({
            employeeId: newEmp._id, employeeName: form.name, department: "Marketing",
  ...
  const openEdit = (emp: Employee) => {
    ...
    setForm({
      ...
      matricule: (emp as any).matricule || "", cnssNumber: (emp as any).cnssNumber || "", address: (emp as any).address || "",
      qualification: (emp as any).qualification || "", category: (emp as any).category || "", echelon: (emp as any).echelon || "",
      situation: (emp as any).situation || "", familyStatus: (emp as any).familyStatus || "",
      numChildren: (emp as any).numChildren != null ? String((emp as any).numChildren) : "", hourlyRate: (emp as any).hourlyRate ? String((emp as any).hourlyRate) : "",
      cin: (emp as any).cin || "",
    });
  ...
  const handleEdit = async () => {
    ...
    try {
      setSubmitting(true); setFormError("");
      await marketingService.updateEmployee(selected!._id, {
        name: form.name, position: form.position, phone: form.phone,
        salary: Number(form.salary) || 0, joinedDate: form.joinedDate,
        ...buildPayrollPayload(),
      });
  ...
  const handleApprove = async (emp: Employee) => {
    ...
    try {
      const res  = await import("@/services/api").then(m => m.default.post(`/marketing/employees/${emp._id}/approve`));
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight leading-none">
              {t("marketing")} <span className="text-[#c8202f]">{t("employees")}</span>
            </h1>
        ...
        <div className={`${card} px-6 py-5`}>
          <div className="flex flex-col md:flex-row md:items-center gap-4">
            <div className="flex items-center gap-3 flex-1">
              ...
              <div>
                <p className="text-sm font-bold text-gray-900 dark:text-white">Annual Marketing Budget</p>
                <p className="text-xs text-gray-500 mt-0.5">
                  {budgetCurrent && budgetCurrent > 0
                    ? <span>Current: <span className="text-[#c8202f] font-bold">{budgetCurrent.toLocaleString()} TND</span></span>
                    : "No budget set yet for this year"
                  }
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-1.5">
                <button onClick={() => { setBudgetYear(y => y - 1); fetchBudget(budgetYear - 1); }}
                  className="w-7 h-7 rounded-lg border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400 flex items-center justify-center text-xs transition">‹</button>
                <span className="text-sm font-bold w-12 text-center">{budgetYear}</span>
                <button onClick={() => { setBudgetYear(y => y + 1); fetchBudget(budgetYear + 1); }}
                  className="w-7 h-7 rounded-lg border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/5 text-gray-400 flex items-center justify-center text-xs transition">›</button>
              </div>
              {/* Budget input */}
              <input
                type="text" inputMode="numeric" placeholder="Enter annual budget (TND)"
                value={annualBudget}
                onChange={e => setAnnualBudget(e.target.value)}
                className="px-3 py-2 bg-gray-100 dark:bg-black/30 border border-gray-200 dark:border-white/10 rounded-xl text-sm text-gray-900 dark:text-white focus:outline-none focus:border-[#c8202f]/50 transition w-52"
              />
              <button onClick={handleSaveBudget} disabled={budgetSaving || !annualBudget}
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#c8202f] hover:bg-[#e02d3c] disabled:opacity-40 text-black font-bold text-xs transition">
                {budgetSaving ? <Loader2 size={13} className="animate-spin" /> : budgetSaved ? <CheckCircle size={13} /> : <Save size={13} />}
              ...
              {pendingEmployees.map((emp, i) => (
                <motion.div key={emp._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">{emp.position} · Marketing</p>
                  </div>
          ...
          <Modal title="Add Employee" onClose={() => setShowCreate(false)}>
            <div className="space-y-4">
              ...
              <div className="pt-2 border-t border-gray-200 dark:border-white/10">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><label className={labelClass}>Matricule</label><input className={inputClass} placeholder="ex. 001234" maxLength={12} value={form.matricule} onChange={e => setForm(f => ({ ...f, matricule: e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "") }))} /></div>
                  <div><label className={labelClass}>N° CNSS</label><input className={inputClass} placeholder="ex. 1234567890" maxLength={12} value={form.cnssNumber} onChange={e => setForm(f => ({ ...f, cnssNumber: e.target.value.replace(/\D/g, "") }))} /></div>
                </div>
                <div className="mt-3"><label className={labelClass}>Adresse</label><input className={inputClass} placeholder="Rue, ville, code postal" maxLength={120} value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} /></div>
