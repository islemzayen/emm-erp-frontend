...
export default function AdminStockPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      setSubmitting(true);
      setFormError("");
      const result = await stockService.createEmployee({
        ...form,
  ...
  const handleEdit = async () => {
    ...
    try {
      setSubmitting(true);
      setFormError("");
      await stockService.updateEmployee(selected!._id, {
        ...form,
        salary: Number(form.salary) || 0,
      });
      await fetchAll();
  ...
  const handleDelete = async () => {
    try {
      setSubmitting(true);
      await stockService.deleteEmployee(selected!._id);
      await fetchAll();
      setShowDelete(false);
    } catch {
    } finally {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white">
              {t("stockModuleName")} <span className="text-slate-400 dark:text-slate-500">{t("employees")}</span>
            </h1>
          </div>
        ...
        <div className={`${surface} overflow-hidden`}>
          <div className="flex flex-col justify-between gap-4 border-b border-slate-200 px-6 py-5 sm:flex-row sm:items-center dark:border-slate-800">
            <div>
              <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Stock Team</h2>
