...
export default function OnlineSalesEmployees() {
  ...
  const fetchAll = async () => {
    try {
      setLoading(true);
      const [emps, st] = await Promise.all([salesService.getAllEmployees(), salesService.getEmployeeStats()]);
      setEmployees(emps);
      setStats(st);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const created = await salesService.createEmployee({
      ...
      if (newEmp?._id) {
        try {
          await documentService.upload({
            employeeId: newEmp._id, employeeName: form.name, department: "Online Sales",
