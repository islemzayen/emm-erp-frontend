...
import { commercialService } from "@/services/admin/commercialService";
...
const POSITIONS = ["Employee", "Commercial Manager", "Warehouse Operator"];
...
export default function AdminCommercialPage() {
  ...
  const generateOtp = useCallback(async () => {
    setOtpLoading(true);
    try {
      const result = await commercialDocumentService.generateOtp();
      setOtp(result);
  ...
  const fetchAll = async () => {
    try {
      ...
      const [emps, st] = await Promise.all([
        commercialService.getAllEmployees(),
        commercialService.getStats(),
      ]);
      setEmployees(emps);
      setStats(st);
    } catch {
  ...
  const handleCreate = async () => {
    ...
    try {
      setSubmitting(true);
      setFormError("");
      const result = await commercialService.createEmployee({
        ...form,
  ...
  const handleEdit = async () => {
    ...
    try {
      setSubmitting(true);
      setFormError("");
      await commercialService.updateEmployee(selected!._id, {
        ...form,
        salary: Number(form.salary) || 0,
      });
      await fetchAll();
  ...
  const handleDelete = async () => {
    try {
      setSubmitting(true);
      await commercialService.deleteEmployee(selected!._id);
      await fetchAll();
      setShowDelete(false);
    } catch {
    } finally {
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white">
              {t("commercialModuleName")} <span className="text-slate-400 dark:text-slate-500">{t("employees")}</span>
            </h1>
          </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-slate-950 dark:text-white">Code OTP — Suppression de documents Commercial</p>
        ...
        <div className={`${surface} overflow-hidden`}>
          <div className="flex flex-col justify-between gap-4 border-b border-slate-200 px-6 py-5 sm:flex-row sm:items-center dark:border-slate-800">
            <div>
              <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Commercial Team</h2>
